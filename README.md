About the PFWsense tools
========================

In conjunction with src.git, ports.git, core.git and plugins.git they
create sets, packages and images for the PFWsense project.
...
(overwriting standard ports and src):
    ...
    # pkg install git
    # cd /usr
    # git clone https://github.com/pfwsense/tools
    # cd tools
    # make update

Note that the PFWsense repositories can also be setup in a non-/usr directory
by setting ROOTDIR.  For example:

    # mkdir -p /tmp/pfwsense
    # cd /tmp/pfwsense
    # git clone https://github.com/pfwsense/tools
    # cd tools
    # env ROOTDIR=/tmp/pfwsense make update
...
Generating a make.conf for use in running PFWsense
--------------------------------------------------

A ports tree in a running PFWsense can be used to build packages
...
Last but not least, a rebuild of PFWsense core and plugins on package
